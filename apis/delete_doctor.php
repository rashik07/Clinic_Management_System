...
class DeleteDoctor{
    ...
    function delete(){
        ...
        {
            try{
                $delete_content = "DELETE FROM doctor WHERE doctor_id='$doctor_id'";
                $result = $conn->exec($delete_content);
                if ($result) {
                    echo json_encode(array("doctor" => "Successful", $status => 1, $message => "Delete Doctor Successful"));
                } else {
                    echo json_encode(array("doctor" => "Error", $status => 0, $message => "Delete Doctor Failed"));
                }
                die();
            }
            catch(Exception $e)
